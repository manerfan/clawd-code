...
export function CondensedLogo() {
  ...
  if ($[9] !== truncatedVersion) {
    t6 = <Box flexDirection="column"><Text>{t5}{" "}<Text dimColor={true}>v{truncatedVersion}</Text></Text><Text dimColor={true}>(Forked From Claude Code, For Study Only)</Text></Box>;
    $[9] = truncatedVersion;
    $[10] = t6;
  } else {
    t6 = $[10];
